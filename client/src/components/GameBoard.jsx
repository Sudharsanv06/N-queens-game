...
import { trackGameStarted, trackGameCompleted, trackGameAbandoned, trackHintUsed, trackAchievementUnlocked } from '../utils/analytics';
import Layout from './Layout';
import './GameBoard.css';

// Function to save game replay for social features
const saveGameReplay = async (gameData, queensPositions) => {
  try {
    const token = localStorage.getItem('token');
    if (!token) return;

    const replayData = {
      title: `${gameData.boardSize}×${gameData.boardSize} ${gameData.mode} - ${gameData.score} points`,
      description: `Completed in ${Math.floor(gameData.timeElapsed / 60)}:${(gameData.timeElapsed % 60).toString().padStart(2, '0')} with ${gameData.moves} moves`,
      gameData: {
        ...gameData,
        queensPositions,
        difficulty: gameData.boardSize <= 6 ? 'easy' : gameData.boardSize <= 8 ? 'medium' : 'hard'
      }
    };

    const response = await fetch(`${import.meta.env.VITE_API_URL}/api/social/replays`, {
      method: 'POST',
      headers: {
    ...
    if (response.ok) {
      console.log('Game replay saved successfully');
    }
  } catch (error) {
    console.error('Failed to save game replay:', error);
  }
};
...
const GameBoard = () => {
  ...
  useEffect(() => {
    ...
    if (user) {
      trackGameStarted({
        mode,
        boardSize: size,
        difficulty: level ? `level-${level}` : (size <= 6 ? 'easy' : size <= 8 ? 'medium' : 'hard'),
        level,
        timeLimit
      });
    }
  }, [size, mode, timeLimit, level, user]);
  ...
  useEffect(() => {
    if (timeLimit && timeLeft > 0 && gameStatus === 'playing') {
      ...
      // Track game abandoned due to timeout
      if (user) {
        trackGameAbandoned({
          mode,
          boardSize: size,
      ...
      if (hasConflicts) {
        ...
        if (newQueens.length === size) {
          ...
          if (user) {
            ...
            // Save game replay for social features
            saveGameReplay(gameData, newQueens);
            
            // Check for achievements
            checkAchievements(gameData).then((result) => {
              if (result.success && result.newUnlocks && result.newUnlocks.length > 0) {
                // Track achievement unlocks
                result.newUnlocks.forEach((unlock) => {
                  trackAchievementUnlocked({
                    id: unlock.achievement.id,
                    name: unlock.achievement.name,
                    category: unlock.achievement.category || 'general',
                    points: unlock.achievement.points || 0
                  });
                });
                
                // Show achievement notification
                result.newUnlocks.forEach((unlock, index) => {
                  setTimeout(() => {
            ...
            if (user.stats) {
              const updatedStats = {
                ...
                totalScore: (user.stats.totalScore || 0) + totalPoints
              };
          ...
          // Trigger leaderboard refresh for all completed games
          if (user) {
            console.log('🎯 Dispatching gameCompleted event for user:', user.name);
  ...
  // Reset game
  const resetGame = () => {
    // Track game abandonment (if not already completed)
    if (user && gameStatus === 'playing' && queens.length > 0) {
      trackGameAbandoned({
        mode,
        boardSize: size,
        timeElapsed: Math.floor((Date.now() - gameStartTime) / 1000),
